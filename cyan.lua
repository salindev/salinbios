...
function Component.invoke(address, method, ...)
    ...
    elseif method == "set" and paletteNotOverwrited and address == gpuAddress then
        ...
        gpu.setPaletteColor(9, 9F3721)
        gpu.setPaletteColor(11, 9F3721)
    end
...
local function bootloader()
    ...
    main.o = function(SELF)
        ...
        if #bootCandidates > 0 then
            ...
            centrizedSet(centerY + 3, "Нет доступных дисков", F, 0xFFFFFF)
        end
    end
...
updateCandidates()
status("Удерживайте CTRL, чтобы оставаться в загрузчике", F, .9, 29, bootloader)
for i = 1, #bootCandidates do
    boot(bootCandidates[i])
end
gpu = configureSystem() and bootloader() or error"Нет доступных дисков"